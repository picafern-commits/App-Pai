...
const STORAGE_KEYS = { trabalhos:'ge_trabalhos_teste', clientes:'ge_clientes_teste', pagamentos:'ge_pagamentos_teste' , lastTrabId:'ge_last_trab_id' };
...
function getManagedUsers(){
  try{
    const stored = JSON.parse(localStorage.getItem('app_users_teste') || 'null');
    if(Array.isArray(stored) && stored.length){
      const fixed = normalizeManagedUsers(stored);
      localStorage.setItem('app_users_teste', JSON.stringify(fixed));
      return fixed;
    }
  }catch{}
  ...
  localStorage.setItem('app_users_teste', JSON.stringify(fixedDefaults));
  return fixedDefaults;
}

function saveManagedUsers(users){
  localStorage.setItem('app_users_teste', JSON.stringify(users));
}
...
function autoBackupInvisible(){
  ...
  localStorage.setItem('ge_invisible_backup_teste', JSON.stringify(payload));
}
