...
import { getFirestore, collection, doc, setDoc, deleteDoc, onSnapshot, getDoc, getDocs, query, writeBatch } from 'https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js';

const APP_VERSION = '1.0.6';
const STORAGE_KEYS = { trabalhos:'ge_trabalhos_mobile_teste', clientes:'ge_clientes_mobile_teste', pagamentos:'ge_pagamentos_mobile_teste' , lastTrabId:'ge_last_trab_id' };
...
function nextTrabalhoIdPreview(){
  const nextId = Number(lastTrabId || 0) + 1;
  return `TRAB-${String(nextId).padStart(4,'0')}`;
}

function nextTrabalhoId(){
  lastTrabId = Number(lastTrabId || 0) + 1;
...
function normalizeTrabalhosIds(){
  ...
  trabalhos = trabalhos.map(t => {
    if(t.workCode || t.ordemId) return { ...t, workCode: t.workCode || t.ordemId };
    changed = true;
    maxExisting += 1;
    return { ...t, workCode: `TRAB-${String(maxExisting).padStart(4,'0')}` };
  });
...
function loadLocal(){
  ...
  const maxExistingId = trabalhos.reduce((max, t) => {
    const match = String(t.id || '').match(/^TRAB-(\d+)$/);
    const n = match ? Number(match[1]) : 0;
    return Math.max(max, n);
  }, 0);

  if(maxExistingId > lastTrabId){
    lastTrabId = maxExistingId;
    localStorage.setItem(STORAGE_KEYS.lastTrabId, String(lastTrabId));
  }
}
function saveLocal(){
...
function getWeeklyBackupPayload(){
  return {
    id: `backup_${new Date().toISOString().replace(/[:.]/g,'-')}`,
    ...
    clientes,
    pagamentos,
    lastTrabId: typeof lastTrabId !== 'undefined' ? lastTrabId : 0,
    counts: {
      trabalhos: Array.isArray(trabalhos) ? trabalhos.length : 0,
...
function updateWeeklyBackupInfo(){
  const status = $('weeklyBackupStatus');
  const info = $('weeklyBackupInfo');
  if(!status || !info) return;

  let meta = null;
  try{ meta = JSON.parse(localStorage.getItem(STORAGE_KEYS.lastWeeklyBackupMeta) || 'null'); }catch{}
  if(meta && meta.createdAt){
    const when = fmtDate(meta.createdAt);
    status.textContent = `Último backup: ${when}`;
    info.textContent = `ID ${meta.id || '-'} • Trabalhos ${meta.counts?.trabalhos ?? 0} • Clientes ${meta.counts?.clientes ?? 0} • Pagamentos ${meta.counts?.pagamentos ?? 0}`;
  }else{
    status.textContent = 'Sem backup ainda';
    info.textContent = 'A app cria uma cópia semanal automática no Firebase quando tiver sessão ativa e sincronização ligada.';
  }
}

async function saveWeeklyBackupToFirebase(force=false){
  if(!syncReady || !firestoreDb) return false;

  const now = Date.now();
  const last = Number(localStorage.getItem(STORAGE_KEYS.lastWeeklyBackup) || 0) || 0;
  const oneWeek = 7 * 24 * 60 * 60 * 1000;

  if(!force && last && (now - last) < oneWeek){
    updateWeeklyBackupInfo();
    return false;
  }

  const payload = getWeeklyBackupPayload();
  await setDoc(doc(firestoreDb, 'backups', payload.id), payload, { merge: true });

  localStorage.setItem(STORAGE_KEYS.lastWeeklyBackup, String(now));
  localStorage.setItem(STORAGE_KEYS.lastWeeklyBackupMeta, JSON.stringify(payload));
  updateWeeklyBackupInfo();
  return true;
}
...
function getManagedUsers(){
  try{
    const stored = JSON.parse(localStorage.getItem('app_users_mobile_teste') || 'null');
    if(Array.isArray(stored) && stored.length){
      const fixed = normalizeManagedUsers(stored);
      localStorage.setItem('app_users_mobile_teste', JSON.stringify(fixed));
      return fixed;
    }
  }catch{}
  ...
  localStorage.setItem('app_users_mobile_teste', JSON.stringify(fixedDefaults));
  return fixedDefaults;
}

function saveManagedUsers(users){
  localStorage.setItem('app_users_mobile_teste', JSON.stringify(users));
}
...
function autoBackupInvisible(){
  ...
  localStorage.setItem('ge_invisible_backup_mobile_teste', JSON.stringify(payload));
}
...
function setRoleUI(){
  ...
  const configNavButtons = [...document.querySelectorAll('.nav-btn[data-tab="configuracoes"], .bottom-btn[data-tab="configuracoes"], .mobile-side-btn[data-tab="configuracoes"]')];
  configNavButtons.forEach(btn => btn.style.display = isMasterAdmin() ? '' : 'none');
  const configPage = $('configuracoes-page');
  if(configPage) configPage.style.display = isMasterAdmin() ? '' : 'none';
  const masterModeSwitchPanel = $('masterModeSwitchPanel');
  if(masterModeSwitchPanel) masterModeSwitchPanel.classList.toggle('hidden', !isMasterAdmin());
  const firebaseBackupRestorePanel = $('firebaseBackupRestorePanel');
  if(firebaseBackupRestorePanel) firebaseBackupRestorePanel.style.display = isMasterAdmin() ? 'block' : 'none';
  ...
  try{
    ...
    onAuthStateChanged(firebaseAuth, user => {
      if (user) {
        ...
        saveWeeklyBackupToFirebase(false).catch(err => console.error('weekly backup error:', err));
      } else {
        syncReady = false;
        setSyncMessage('Sem sessão Firebase', 'bad');
      }
      if (currentRole) renderAll();
  if(!isMasterAdmin() && document.querySelector('.page.active')?.id === 'configuracoes-page'){ switchTab('dashboard'); }
    });
  }catch(err){
...
function attachRealtimeListeners(){
  ...
  bind('trabalhos', rows => { trabalhos = rows.map(r => ({ ...r, workCode: r.workCode || r.ordemId || '' })); normalizeTrabalhosIds(); });
...
function renderTrabalhos(){
  ...
  $('trabalhosTableBody').innerHTML = rows.length ? rows.map(t=>{
    ...
    return `<tr><td><strong>${escapeHtml(t.workCode||'-')}</strong></td><td>${escapeHtml(t.cliente||'-')}</td><td>${escapeHtml(t.tipoTrabalho||'-')}</td><td>${euro(t.valor||0)}</td><td>${fmtDate(t.dataInicio)}</td><td>${fmtDate(t.dataFim)}</td><td><span class="badge" data-state="${escapeHtml(t.estado||'-')}">${escapeHtml(t.estado||'-')}</span></td><td><span class="invoice-badge ${invoiceClass}">${escapeHtml(invoiceType)}</span></td><td><div class="row-actions">${trabalhoActions(t)}</div></td></tr>`;
...
function exportBackup(){ const payload={ exportadoEm:new Date().toISOString(), appVersion:APP_VERSION, currentUsername, currentRole, trabalhos, clientes, pagamentos, lastTrabId: typeof lastTrabId !== 'undefined' ? lastTrabId : 0 }; const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='gestao-empresa-backup.json'; a.click(); URL.revokeObjectURL(a.href); }
$('exportBackupBtn')?.addEventListener('click', exportBackup);
$('runWeeklyBackupBtn')?.addEventListener('click', async ()=>{
  try{
    if(!syncReady){ alert('A app ainda não está ligada ao Firebase.'); return; }
    await saveWeeklyBackupToFirebase(true);
    alert('Backup Firebase criado com sucesso.');
  }catch(err){ console.error(err); alert('Erro ao criar backup no Firebase.'); }
});
