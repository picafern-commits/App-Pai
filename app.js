...
function switchTab(tab){
  ...
  pageTitle.textContent = document.querySelector(`.nav-btn[data-tab="${tab}"]`)?.textContent || 'Dashboard';
...
function renderTrabalhos(){
  ...
  const filtered = trabalhos.filter(t => {
    const matchTerm = !term || t.cliente.toLowerCase().includes(term) || t.tipoTrabalho.toLowerCase().includes(term);
...
function renderClientes(){
  ...
  const filtered = clientes.filter(c => !term || c.nome.toLowerCase().includes(term) || (c.telefone||'').toLowerCase().includes(term));
